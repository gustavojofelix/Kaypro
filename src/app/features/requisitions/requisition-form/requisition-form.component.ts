...
@Component({
  ...
  template: `
    <div class="mb-6">
      <h2 class="text-2xl font-bold text-gray-800">Nova Requisição</h2>
    </div>
    ...
    <div class="bg-white shadow sm:rounded-lg px-4 py-5 sm:p-6">
      <form [formGroup]="form" (ngSubmit)="onSubmit()">
        ...
        <!-- Combustivel Fields -->
        <div *ngIf="form.get('type')?.value === 'COMBUSTIVEL'" class="bg-gray-50 p-4 rounded-md mb-6 border border-gray-200">
          <h3 class="text-lg font-medium text-gray-900 mb-4">Detalhes do Abastecimento</h3>
        ...
        <!-- Material Fields -->
        <div *ngIf="form.get('type')?.value === 'MATERIAL'" class="bg-gray-50 p-4 rounded-md mb-6 border border-gray-200">
           <h3 class="text-lg font-medium text-gray-900 mb-4">Itens de Material</h3>
           ...
           <div formArrayName="items">
             <div *ngFor="let item of items.controls; let i=index" [formGroupName]="i" class="flex items-end gap-4 mb-4">
               <div class="flex-1">
               ...
               <div>
                 <button type="button" (click)="removeItem(i)" class="mb-1 p-2 text-red-600 hover:text-red-800 bg-red-50 rounded-md">
           ...
           <button type="button" (click)="addItem()" class="mt-2 inline-flex items-center px-3 py-2 border border-gray-300 shadow-sm text-sm leading-4 font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">
             + Adicionar Item
           </button>
           ...
           <div class="mt-6 border-t pt-4 flex justify-end">
             <div class="text-right">
               ...
               <span class="ml-2 text-xl font-bold text-gray-900">{{ calculateMaterialTotal() | currency:'MZN':'symbol-narrow' }}</span>
             </div>
           </div>
        </div>

        <div class="flex justify-end pt-5 border-t border-gray-200 gap-3">
          <button type="button" routerLink="/requisitions" class="bg-white py-2 px-4 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">
            Cancelar
          </button>
          <button type="submit" [disabled]="form.invalid || kmError" class="bg-blue-600 py-2 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium text-white hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 disabled:opacity-50">
            Submeter Requisição
          </button>
        </div>
      </form>
