...
@Component({
  ...
  template: `
    <div class="mb-6 flex justify-between items-center">
      <h2 class="text-2xl font-bold text-gray-800">Aprovação Administração (Nível 1)</h2>
    </div>
      ...
      <table class="min-w-full divide-y divide-gray-200">
        ...
        <tbody class="bg-white divide-y divide-gray-200">
          <tr *ngFor="let req of pendingReqs$ | async">
            ...
            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 font-semibold">{{req.totalValue | currency:'MZN':'symbol-narrow'}}</td>
          ...
          <tr *ngIf="(pendingReqs$ | async)?.length === 0">
            <td colspan="5" class="px-6 py-4 whitespace-nowrap text-sm text-center text-gray-500">Nenhuma requisição pendente.</td>
          </tr>
        </tbody>
      </table>
    </div>
